    ...
    fetch("/rblxfaces.json")
        .then(r => {
            r.text().then(tt => {
                ...
                divideRes.forEach(a => {
                    fetch("https://corsfix.com/?https://thumbnails.roblox.com/v1/assets?assetIds=" + JSON.stringify(a).replace("[", "").replace("]", "") + "&returnPolicy=PlaceHolder&size=512x512&format=Png&isCircular=false", { mode: "no-cors", headers: { "Accept": "application/json" } })
                        .then(rr => {
                            if (rr.status != 200) {
